...
public class TelaEditarClientePJ {
	...
	private void acaoSalvar() throws SQLException {
		...
		Alert alert = new Alert(AlertType.CONFIRMATION, "Deseja realmente atualizar os dados do CLIENTE PJ ?", sim,
				nao);
		...
		if (result.get().equals(sim)) {
			...
			Pessoa pessoa = new Pessoa(telaPrincipalClientes.getIdPessoa(), nome, cnpj, null, null, null, enderecos, telefones, emails, tipo,
					ativo, null);

			this.pessoaDAO = Principal.getPessoaDAO();
			boolean sucesso = pessoaDAO.atualizarPJ(pessoa);
			if (sucesso) {
